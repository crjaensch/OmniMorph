...
def ai_suggest(
    sql: str, error_msg: str, schema_txt: str, source: Union[str, Path] = None
) -> str:
...
def _ensure_avro_extension(con):
    ...
    try:
        ...
        result = con.execute(
            "SELECT * FROM duckdb_extensions() WHERE extension_name = 'avro' AND loaded"
        ).fetchall()
        if result:
...
def _configure_azure_credentials(con, azure_credentials):
    ...
    if azure_credentials.get("azure_connection_string"):
        con.execute(
            f"SET azure_storage_connection_string='{azure_credentials['azure_connection_string']}'"
        )
    elif azure_credentials.get("azure_account_name") and azure_credentials.get(
        "azure_account_key"
    ):
        con.execute(
            f"SET azure_account_name='{azure_credentials['azure_account_name']}'"
        )
        con.execute(f"SET azure_account_key='{azure_credentials['azure_account_key']}'")
    elif all(
        azure_credentials.get(k)
        ...
        con.execute(
            f"SET azure_client_secret='{azure_credentials['azure_client_secret']}'"
        )
...
def _ensure_azure_extension(con):
    ...
    try:
        # Check if the extension is already installed and loaded
        con.execute(
            "SELECT loaded FROM duckdb_extensions() WHERE extension_name='azure'"
        )
        result = con.fetchone()
...
def _register_source(con, fmt, source, *, lazy=False):
    ...
    if fmt == Format.PARQUET:
        ...
        if avro_extension_loaded:
            ...
            try:
                con.execute(
                    f"CREATE VIEW {name} AS SELECT * FROM read_avro('{source}')"
                )
                return
            except duckdb.Error:
        ...
        try:
            ...
            raise QueryError(
                f"The fastavro package is required for Avro support: {str(imp_err)}"
            ) from imp_err
        except Exception as exc:
            raise QueryError(
                f"Failed to read Avro file using fastavro: {str(exc)}"
            ) from exc

    elif fmt == Format.XLSX:
        # ------------------------------------------------------------------
        # Excel via pandas -> PyArrow -> DuckDB view
        # ------------------------------------------------------------------
        import pandas as pd
        import pyarrow as pa
        from omni_morph.data.filesystems import FileSystemHandler
        ...
        try:
            ...
            raise QueryError(f"Failed to read Excel file {source}: {exc}") from exc

    else:
        raise QueryError(f"Unsupported source file: {source}")
