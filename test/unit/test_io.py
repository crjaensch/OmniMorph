...
def assert_table_equal_pandas(table1: pa.Table, table2: pa.Table):
    ...
    pd.testing.assert_frame_equal(
        df1.sort_index(axis=1), df2.sort_index(axis=1), check_dtype=True
    )
...
def test_read_csv(temp_dir_module, sample_table):
    ...
    read_options = pacsv.ReadOptions(
        use_threads=False
    )  # Autogenerate column names off by default
...
@pytest.mark.skipif(
    not HAS_FASTAVRO_TEST, reason="Test requires fastavro to be installed"
)
...
@pytest.mark.skipif(
    MODULE_HAS_FASTAVRO, reason="Test requires fastavro to be NOT installed"
)
...
def test_write_csv(temp_dir_module, sample_table):
    ...
    read_back_table = pacsv.read_csv(
        file_path, read_options=read_options, convert_options=convert_options
    )
...
def test_write_empty_table(temp_dir_module, empty_table):
    ...
    for fmt in formats_to_test:
        ...
        if fmt in [Format.PARQUET, Format.AVRO]:
            assert read_back_table.schema.equals(
                empty_table.schema, check_metadata=False
            )
...
def test_generate_avro_schema_mixed_types():
    ...
    try:
        mixed_table_inferred = pa.Table.from_pandas(
            mixed_df, safe=False
        )  # safe=False might be needed for object dtype
    ...
    mixed_table = mixed_table_inferred.set_column(
        col_index, "col_mix", mixed_table_inferred.column("col_mix").cast(pa.string())
    )
